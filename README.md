...
分层结构中每一层都有一个唯一标识，比如数据链路层的 MAC 地址，网络互联层的 IP 地址和传输层的端口号。**端口号**用来区分主机上的应用程序。
...
**全双工**指的是客户端和服务端既可以发送数据也可以接收数据。

**超时重传**是当发生丢包时，发送方会对其进行重传，重传时间间隔是指数级退避，直到120s为止，重传次数为15次。
...
**Nagle算法**的作用是减少小包在客户端和服务端的直接传输。当一个TCP连接中有**在传数据**时，小于MSS的报文段就不能被发送，直到所有的在传数据都收到ACK后，
此时TCP还不会立即发送数据，而是会收集小包一起发送，代价是会有一些延迟。

**延迟确认**的原理和Nagle算法一致，在服务端收到报文时并不是立即回复ACK，因为单独发送ACK太浪费了。等待一段时间（Linux 40ms），如果在该期间有其他报文发送，
就可以和ACK一起发送过去了。
...
当发送端发送SYN报文想建立一条连接时，会开启连接建立定时器，如果没有收到对端的ACK报文时，将对SYN报文进行重传，超过重传次数后，
会抛出 `Connection timed out` 异常
...
该定时器在延时确认时生效，收到报文时并不会立即回复ACK报文，而是等待一段时间，如果有数据发送会将ACK报文一并发送
...
FIN-WAIT-2 定时器在发送FIN报文并收到ACK报文后开始计时。如果对端一直不发送FIN报文，那么超时后会关闭连接，否则的话该连接会一直处于 `FIN-WAIT-2` 状态。

#### 7. TIME_WAIT定时器

也叫2MSL定时器，主动关闭的一端在TIME_WAIT持续2连个MSL时常，超时后端口被安全的重用。
...
- `telnet [ip] [port]` 和 `nc -v [ip] [port]`: 判断远端 server 指定的端口是否可达
